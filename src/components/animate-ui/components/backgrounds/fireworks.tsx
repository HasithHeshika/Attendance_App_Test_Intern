...
function FireworksBackground({
  ...
  React.useEffect(() => {
    ...
    let launchTimeoutId: ReturnType<typeof setTimeout>;

    // Hard caps prevent a runaway feedback loop: spawning is wall-clock based
    // (setTimeout) while motion/decay is per-frame, so on any frame-rate dip the
    // particles outlive their spawn rate, pile up, and each frame gets heavier — a
    // death spiral that freezes the tab after a few seconds. Bounding the work per
    // frame keeps it smooth and crash-free regardless of device speed.
    const MAX_FIREWORKS = 14;
    const MAX_PARTICLES = 900;

    const handleExplosion = (particles: ParticleType[]) => {
      const room = MAX_PARTICLES - explosions.length;
      if (room <= 0) return;
      explosions.push(...(particles.length > room ? particles.slice(0, room) : particles));
    };
